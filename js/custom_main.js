    ...
    function accordionSlider() {
      ...
      $(".accordion-slider .box").on("mouseenter", function() {
        if (!$(this).hasClass("active") && $(window).width() >= 550) {
          ...
          if (active) {
            TweenLite.to(active.find(".accordion-slider-title"), 0.3, {
              opacity: 0,
              x: 0,
              overwrite: "all"
            });
            TweenLite.to(active.find(".accordion-slider-content"), 0.3, {
              opacity: 0,
              x: 0,
              overwrite: "all"
            });
          ...
          tl
            .to(
              others,
              0.8,
              { ease: Back.easeOut.config(1.1), width: collapsedWidth + "%" },
              0
            )
            .to(
              active,
              0.8,
              { ease: Back.easeOut.config(1.1), width: openWidth + "%" },
              0
            )
            .to(
              active.find(".accordion-slider-title"),
              0.6,
              { ease: Back.easeOut.config(1.2), opacity: 1, x: 100 },
              0.3
            )
            .to(
              active.find(".accordion-slider-content"),
              0.6,
              { ease: Back.easeOut.config(1.2), x: 100, opacity: 1 },
              0.4
            );
        }
      });
        ...
        if ($(window).width() >= 550) {
          ...
          tl
            .to(
              all,
              0.8,
              { ease: Back.easeOut.config(1.1), width: 100 / boxes + "%" },
              0
            )
            .to(
              active.find(".accordion-slider-title"),
              0.3,
              { opacity: 0, x: 0, overwrite: "all" },
              0
            )
            .to(
              active.find(".accordion-slider-content"),
      ...
      if ($(window).width() < 550) {
        $(".accordion-slider-title, .accordion-slider-content").removeAttr(
          "style"
        );
